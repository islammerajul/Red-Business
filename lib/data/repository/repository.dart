...
class Repository {
  ...
  // Login:---------------------------------------------------------------------
  Future<ResponseObject> login(String userName, String password, String account_type) async {
    return await _appwriteApi.emailSession(userName, password,account_type).then((res) {
  ...
  // Signup:---------------------------------------------------------------------
  Future<ResponseObject> signup(String userName, String email, String password, String teams_id, List<String> batch_id,) async {
    return await _appwriteApi.signupSession(userName, email, password, teams_id, batch_id).then((res) {
  ...
  // Future<ResponseObject> addPayment(doc) async {
  //   print("repodata:::${doc.toString()}");
  //   return await _appwriteApi.addPayment(doc).then((res) {
  //     print("resource : $res");
  //     return res;
  //   }).catchError((error) {
  //     return ResponseObject(id: ResponseCode.FAILED, object: "Try again");
  //   });
  // }
  Future<ResponseObject> addStudentFunction(email,teams_id,batch_id,url,institution,user,password) async {
    print("repo email:::${email}");
    return await _appwriteApi.addStudentWithFunction(email,teams_id,batch_id,url,institution,user,password).then((res) {
      return res;
    }).catchError((error) {
      return ResponseObject(id: ResponseCode.FAILED, object: "Try again");
    });
  }
  Future<ResponseObject> addStudentAuthMail(student_name, institution_name, user_name, pass, email) async {
    print("repo email:::${email}");
    return await _appwriteApi.studentAuthMails(student_name, institution_name, user_name, pass, email).then((res) {
      return res;
    }).catchError((error) {
      return ResponseObject(id: ResponseCode.FAILED, object: "Try again");
    });
  }
  // Future<ResponseObject> studentAuthMailFunction(student_name,institution_name,user_name,pass,email) async {
  //   print("repo email:::${email}");
  //   return await _appwriteApi.studentAuthMail(student_name,institution_name,user_name,pass,email).then((res) {
  //     return res;
  //   }).catchError((error) {
  //     return ResponseObject(id: ResponseCode.FAILED, object: "Try again");
  //   });
